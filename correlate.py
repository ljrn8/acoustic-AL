...
def _compute_mfcc_from_restricted_stft(S_restricted, sr, frequency_range, n_mfcc=13, n_mels=128, ):
    fmin, fmax = frequency_range
    
    # Define the Mel filter bank to match the restricted frequency range
    mel_basis = librosa.filters.mel(sr=sr, n_fft=S_restricted.shape[0]*2-2, n_mels=n_mels, fmin=fmin, fmax=fmax)
...
def cross_correlate(
        reference_file, frequency_range, deployment=1, site=1, sr=None,
        threshold=0.99, output_file=path.join("correlations", "corr_" + datetime.now().strftime("%m.%d.%Y.%H.%M.%S")),
        return_correlations=15, max_recordings=3
    ):
    ...
    for root, _, files in os.walk(data_dir):
        for i, file in enumerate(files):
            if file.endswith(".wav"):
                ...
                S_mfcc_flat = _compute_mfcc_from_restricted_stft(S, sr, frequency_range, n_mels=64).flatten()
                S_ref_mfcc_flat = _compute_mfcc_from_restricted_stft(S_ref, sr, frequency_range, n_mels=64).flatten()
    ...
    **ideally should need to use this
    ...
    with open(correlations_file) as f:
        for line in f:
            ...
            extract_segment(recording_path, output_dir + "corr_" + file[:3] + "_" + str(time) + ".wav",
                            time, time + duration)


def correlate_save_view(reference_data, reference_file,output_file, 
                        return_correlations=30, max_recordings=50):
    
    # read reference data    
    with open(reference_data, "r") as f:
        file, start_s, end_s, name, start_hz, end_hz = f.readline().split()
    
    start_s, end_s, start_hz, end_hz =  [float(i) for i in [start_s, end_s, start_hz, end_hz]]
    ...
    # correlate
    print("\n== Correlating ==")
    corrs = cross_correlate(reference_file, frequency_range, 
                            return_correlations=return_correlations, 
                            max_recordings=max_recordings, output_file=output_file)
    
    # view correlations
    print("\n== Plotting ==")
    plot_correlations(output_file, 
                      reference_wav=reference_file, frequency_lines=frequency_range)
